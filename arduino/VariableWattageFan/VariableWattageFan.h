...
{
  int fan_startup_current = 90;

  class VariableWattageFan : VariableWattageDevice
  {
  public:
    VariableWattageFan(int outputChannel, int low = 60, int high = 75)
    ...
    setPower(int power)
    {
      if (this->isStartingFan == true)
      {
        if (this->fanStartDelay.isExpired())
        {
          ...
          VariableWattageDevice::getPower() == 0 && VariableWattageDevice::getMode() == ON_OFF_typedef::OFF && power > 0)
      {
    ...
    startFromStopped(int power)
    {
      ...
      if (this->turnOffDelay.isExpired())
      {
        ...
        if (power < 50)
        {
          ...
          VariableWattageDevice::setPower(this->mapPower(fan_startup_current));
        }
        else
        {
          VariableWattageDevice::setPower(this->mapPower(power));
